...
float output_interval = 100;  // in milliseconds
...
#define OUTPUT__MODE_ON_BOARD 5// Output mode for using on board
...
int output_mode = OUTPUT__MODE_ON_BOARD;
...
void loop()
{
  ...
  if (Serial.available() >= 2)
  {
    if (Serial.read() == '#') // Start of new control message
    {
      ...
      else if (command == 'o') // Set _o_utput mode
      {
		...
		// Output on board the vercal
		else if(output_param == 'z')
		{
			output_mode = OUTPUT__MODE_ON_BOARD;
			output_format = OUTPUT__FORMAT_TEXT;
		}
        //*************************************newly added ******************************************//
  ...
  if((millis() - timestamp) >= calculate_interval)
  {
	...
	else if(output_mode == OUTPUT__MODE_ON_BOARD)
	{
	  // Apply sensor calibration
      compensate_sensor_errors();
